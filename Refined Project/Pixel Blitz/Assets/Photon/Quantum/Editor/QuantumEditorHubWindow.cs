    ...
    protected static List<QuantumEditorHubPage> Pages {
      get {
        if (_pagesInitialized == false) {
          ...
          try {
            var allPages = new List<QuantumEditorHubPage>();
            ...
            // Select all pages with no insert handling
            _pages = allPages.Where(p => p.InsertPageAnchor == QuantumEditorHubPage.PageAnchor.None).ToList();

            for (int i = 0; i < allPages.Count; i++) {
              switch (allPages[i].InsertPageAnchor) {
                case QuantumEditorHubPage.PageAnchor.After: {
                    // Insert after reference page or at the end
                    var index = -1;
                    if (string.IsNullOrEmpty(allPages[i].InsertPageReference) == false) {
                      index = _pages.FindIndex(p => string.Equals(p.Title, allPages[i].InsertPageReference, StringComparison.Ordinal));
                    }
                    if (index >= 0) {
                      _pages.Insert(index + 1, allPages[i]);
                    } else {
                      _pages.Add(allPages[i]);
                    }
                  }
                  break;
                case QuantumEditorHubPage.PageAnchor.Before: {
                    // Insert before reference page or at the beginning
                    var index = -1;
                    if (string.IsNullOrEmpty(allPages[i].InsertPageReference) == false) {
                      index = _pages.FindIndex(p => string.Equals(p.Title, allPages[i].InsertPageReference, StringComparison.Ordinal));
                    }
                    if (index >= 0) {
                      _pages.Insert(index, allPages[i]);
                    } else {
                      _pages.Insert(0, allPages[i]);
                    }
                  }
                  break;
                case QuantumEditorHubPage.PageAnchor.Overwrite: {
                    // Overwrite or insert at the end
                    var index = _pages.FindIndex(p => string.Equals(p.Title, allPages[i].InsertPageReference, StringComparison.Ordinal));
                    if (index >= 0) {
                      _pages[index] = allPages[i];
                    } else {
                      _pages.Add(allPages[i]);
                    }
                  }
